...
/*
사과중에 녹색이면서 당도가 9이상인 사과만 선별하여 // filter
'이 사과는 xx색이며 당도는 xxx입니다' // map
...
// const mappedApples = appleBasket.filter(apple => apple.color === 'green' && apple.sweet >= 9).map(apple => {
...
appleBasket
  ...
  .map(apple => {const covertedColor = apple.color === 'green' ? '초록' : '빨강';
  return `이 사과는 ${covertedColor}색이며 당도는 ${apple.sweet}입니다.`;
...
/*
회원목록에서 서울에 사는 회원들의 두번쨰 취미만 추출하여
