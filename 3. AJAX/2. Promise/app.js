
// 서버 URL
const URL = 'https://jsonplaceholder.typicode.com/posts';
const URL2 = 'http://localhost:8383/api/v1/replies/30';

// ul태그 가져오기
const $postUl = document.querySelector('.posts')

// 화면에 게시물을 렌더링하는 함수
const renderPosts = postList => {

  postList.forEach(post => {
      // 게시물 하나를 li태그로 만들어서 ul에 집어넣기
      // 템플릿 태그 가져오기
      const $template = document.getElementById('single-post');
      // 템플릿 태그에서 li 태그 추출하기
      const $li = document.importNode($template.content, true); // false로 하면 자식태그 제외 li만 가져옴
      $li.querySelector('h2').textContent = post.title;
      $li.querySelector('p').textContent = post.body;

      $postUl.appendChild($li)
  })
};



// 서버에서 게시물 정보 받아오기
fetch(URL2)
    .then(res => res.json()) // respones 에서 data를 json 으로 추출
    .then(json => {
        console.log(json) // url에 있는 데이터들을 모조리 json 형식으로 출력한다.
        // 게시물 정보 화면에 그리기
        renderPosts(json);
    });


